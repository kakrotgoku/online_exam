...
class Exam extends AppModel
{
    ...
    public function getPurchasedExam($type, $studentId, $currentDateTime, $limit = 0)
    {
        ...
        $examList = $this->find('all', array(
            ...
            'conditions' => array($start_date, $end_date, $expiredDate, 'Exam.status' => 'Active', 'Exam.user_id' => 0, 'Payment.student_id' => $studentId, 'StudentGroup.student_id' => $studentId, 'Package.package_type <>' => 'F'),
    ...
    public function getFreeExam($studentId, $currentDateTime, $limit = 0)
    {
        $examLimit = "";
        $start_date = array('Exam.start_date <=' => $currentDateTime);
        $end_date = array('Exam.end_date >' => $currentDateTime);
        ...
        $examList = $this->find('all', array(
            ...
            'joins' => array(array('table' => 'exam_questions', 'alias' => 'ExamQuestion', 'type' => 'LEFT', 'conditions' => array('Exam.id=ExamQuestion.Exam_id')),
                ...
                array('table' => 'exam_groups', 'alias' => 'ExamGroup', 'type' => 'Inner', 'conditions' => array('Exam.id=ExamGroup.exam_id')),
                array('table' => 'student_groups', 'alias' => 'StudentGroup', 'type' => 'Inner', 'conditions' => array('StudentGroup.group_id=ExamGroup.group_id')),
                array('table' => 'payments', 'alias' => 'Payment', 'type' => 'LEFT', 'conditions' => array('StudentGroup.student_id=Payment.student_id')),
                ...
                array('table' => 'packages', 'alias' => 'Package', 'type' => 'LEFT', 'conditions' => array('Package.id=ExamsPackage.package_id', 'Package.id=PackagesPayment.package_id')),

            ),
            'conditions' => array($start_date, $end_date, $expiredDate, 'Exam.status' => 'Active', 'Exam.user_id' => 0, 'Package.package_type' => 'F', 'StudentGroup.student_id' => $studentId),
            'order' => array('Exam.start_date' => 'asc'),
            'group' => array("Exam.id"),
            'limit' => $examLimit));
        $this->virtualFields = array();
    ...
    public function insertFreeExam($studentId, $currentDateTime, $limit = 0)
    {
        $examLimit = "";
        $start_date = array('Exam.start_date <=' => $currentDateTime);
        $end_date = array('Exam.end_date >' => $currentDateTime);
