...
.optionSerial{float: left;margin-top: 2px;}
i.material-icons..clear{color: red;}
i.material-icons.check {color: #26ce26;}
.tab-content {color: #191919;}

.me{background: #08bbe2;padding: 5px;color: #fff}
img.img-thumbnail {
    border-radius: 100%;
    padding: 2px;
}
...
button.btn.btn-default.btn-sm.btn-block {font-size: 18px;}
...
.meheding>h3,h4 {padding: 0; margin:0;}

.myrank {float: right;padding: 15px;}
.myrank.comp {padding: 0;}
.rank {color: #fff;}
h4.panel-title {
    padding: 20px;
}
i.material-icons.navigate_next {font-size: 35px;}
i.material-icons.navigate_before {font-size: 35px;}
.navigateNext{
background: #FF4081;
    height: 58px;
    color: #fff;
...
function callNext(quesNo){if($('#totalQuestion').text()!=quesNo)quesNo++;$('.exam-panel').hide();$('#quespanel'+quesNo).show();$("html, body").animate({ scrollTop: 0 }, 600);
    return false;}
...
<div class="row my-result">
	<div class="col-md-12" style="padding: 0;">
			...
			<div class="">
				...
				<thead>
					...
					<tr>                                    
						...
						<td data-th="Correct/Incorrect Question: "><span class="text-success"><?php echo$userValue['Subject']['correct_question'];?></span>/<span class="text-danger"><?php echo$userValue['Subject']['incorrect_question'];?></span></td>
						<td data-th="Marks Scored/Negative Marks: "><span class="text-success"><?php echo$userValue['Subject']['marks_scored'];?></span>/<span class="text-danger"><?php echo$userValue['Subject']['negative_marks'];?></span></td>
						<td data-th="Unattempted Questions/Marks: "><span class="text-warning"><?php echo$userValue['Subject']['unattempted_question'];?></span>/<span class="text-danger"><?php echo$userValue['Subject']['unattempted_question_marks'];?></span></td>
					</tr>
		...
		<div class="tab-pane" id="solution">
			...
			<div class="col-sm-9">
			<?php foreach($post as $k=>$ques):$quesNo=$ques['ExamStat']['ques_no'];?>
				<div class="exam-panel" style="margin-top: 10px;box-shadow: 0px 2px 5px #888888;" id="quespanel<?php echo$quesNo;?>">
					<div class="panel-group" style=";" id="accordion">
					<div class="panel panel-default">
					<div class="panel-heading" style="background: #08BBE2;color: #fff;">
					<h4 class="panel-title"><a data-toggle="collapse" data-parent="#accordion"><strong><?php echo __('Question Report For');?>  </strong><?php echo h($examDetails['Exam']['name']);?></a></h4>
					</div>
					</div>
					</div>
					<div class="">
						<table id="cart" class="table table-condensed">
							...
							?>
							<tr class="<?php echo$quesStatus;?>">
							<tr><td colspan="3"><?php echo '<strong>'.__('Question').': '.$quesNo.'</strong><br>&nbsp;&nbsp;'.str_replace("<script","",$ques['Question']['question']);?></td></tr>
							<tr><td colspan="3">
							<?php
							if($ques['Qtype']['type']=="M")
							{
								...
								if(strlen($ques['Question']['answer'])>1)
								{
									...
									if($opt==$value && $ques['ExamStat']['ques_status']=='W'){$incorrectImg='<i class="material-icons clear">clear</i>';break;}}
									unset($value);
									foreach(explode(",",$ques['ExamStat']['correct_answer']) as $value){
									if($opt==$value){$incorrectImg='<i class="material-icons clear">clear</i>';break;}}
									unset($value);
								}
								else
								{
									if($opt==$ques['ExamStat']['correct_answer']){$correctImg='<i class="material-icons check">check</i>';}else{$correctImg="";}
									if($opt==$ques['ExamStat']['option_selected'] && $ques['ExamStat']['ques_status']=='W'){$incorrectImg='<i class="material-icons clear">clear</i>';}else{$incorrectImg="";}
								}
							...
							<tr>
								...
								<?php if($ques['ExamStat']['ques_status']==NULL && !$ques['ExamStat']['answer'])echo'<div style="border: 1px solid;padding: 15px 15px;width: 100%;"><strong class="">'.__('Not Attempt').'</strong></div>';else echo'<div style="border: 1px solid;padding: 15px 15px;width: 100%;"><span class="">'.__('Attempt').'</span></div>';?></strong></td>
								<?php if($ques['ExamStat']['ques_status']=='R'){?>
									<td style="margin: 0;padding-top: 0;    padding-bottom: 0;line-height: 0;"><div style="border: 1px solid;padding: 15px 15px;width: 100%;"><strong class=""><?php echo __('Correct');?></strong></div></td><?php }?>
								<?php if($ques['ExamStat']['ques_status']=='W'){?>
									<td style="margin: 0;padding-top: 0;    padding-bottom: 0;line-height: 0;">
									<div style="border: 1px solid;padding: 15px 15px;width: 100%;"><strong class=""><?php echo __('Incorrect');?></strong></div></td>
								<td>
								<div style="float: left;border-right: 1px solid;padding: 5px 15px;width: 50%;border: 1px solid;">
								
								<?php echo __('Your Answers');?><br><strong class=""><?php echo __($userAnswer);?></strong>
								</div>
								<div style="float: right;border-right: 1px solid;padding: 5px 15px;width: 50%;border-top: 1px solid;border-bottom: 1px solid;">
								<?php if($ques['Qtype']['type']!="S"){?><?php echo __('Correct Answer');?> <br><?php echo __($correctAnswer);?></strong><?php }?></td>
								</div>
								<?php }else{?>
								<?php if($ques['Qtype']['type']!="S"){?>
									<td><div style="float: left;border-right: 1px solid;padding: 5px 15px;width: 100%;border: 1px solid;"><?php echo __('Correct Answer');?> <br></strong>&nbsp;<strong class=""><?php echo __($correctAnswer);?></script","",$ques['Question']['question']);?></div></td>
									<?php }}?>
							...
							<tr>
								<td>
									<div style="float: left;border-right: 1px solid;padding: 5px 15px;width: 50%;border: 1px solid;">
										<?php echo __('Max Marks');?> <br><strong><?php echo$ques['ExamStat']['marks'];?></strong>
									</div>
									<div style="float: right;border-right: 1px solid;padding: 5px 15px;width: 50%;border-top: 1px solid;border-bottom: 1px solid;">
									<?php echo __('Marks Scored');?><br><strong>
									<?php if (!empty($ques['ExamStat']['marks_obtained'])) {echo $ques['ExamStat']['marks_obtained'];} else{echo 0;} ;?></strong>
									</div>
									<div style="clear: both;"></div>
									<div style="border-right: 1px solid;border-left: 1px solid;padding: 5px 15px;width: 100%;border-bottom: 1px solid;"><?php echo __('Time Taken');?> <br><strong><?php echo $this->Function->secondsToWords($ques['ExamStat']['time_taken'],'-');?></strong></div>
								</td>
			...
			<br>
			<div class="col-sm-3">
				<div class="panel-group" style="overflow: hidden;padding: 10px;box-shadow: 0px 2px 5px #888888;" id="accordion">
		...
		<div class="tab-pane active" id="compare-report">
			<div class="com-md-12 col-sm-12 col-xs-12">
				<div style="margin-top: 10px;box-shadow: 0px 2px 5px #888888;">
					...
					<h4 class="panel-title"><a data-toggle="collapse" data-parent="#accordion"><strong><?php echo __('Compare Report For');?>  </strong><?php echo h($examDetails['Exam']['name']);?></a></h4>
						...
						<div class="">
							<table class="table">
								...
								<tr>
									...
									<td><strong class=""><?php echo $examDetails['Result']['obtained_marks'];?>/<?php echo $examDetails['Result']['total_marks'];?></strong></td>
								</tr>
								<tr>
						...
						<div class="col-md-3">
							<div class="">
								<table class="table">
									...
									<tr>
										<td><?php echo __('Total Score');?></td>
										<td><strong class=""><?php echo $compPost[0]['Result']['obtained_marks'];?>/<?php echo $compPost[0]['Result']['total_marks'];?></strong></td>
									</tr>
									<tr>
										<td><?php echo __('Percentage');?></td>
							...
							<?php if($k<$compareCount){echo$this->Form->button(__('<i class="material-icons navigate_next">navigate_next</i>'),array('onClick'=>"callCompareNext($k);",'class'=>'btn btn-default btn-sm btn-block navigateNext'));}?>
							</div>
							<div class="col-sm-2" style="position: fixed;bottom: 0;right: 70px;">
								<?php if($k!=0){echo$this->Form->button(__('<i class="material-icons navigate_before">navigate_before</i>'),array('onClick'=>"callComparePrev($k);",'class'=>'btn btn-default btn-sm btn-block navigatePrev '));}?>
							</div>
						</div>
