            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered">
                    ...
                    <?php foreach ($Homesections as $post):
                        ...
                        ?>
                        <tr>
                            <!-- <td><?php //echo $this->Form->checkbox(false, array('value' => $post['Homesection']['id'], 'name' => 'data[Homesection][id][]', 'id' => "DeleteCheckbox$id", 'class' => 'chkselect')); ?></td> -->
                            <td><?php echo $serialNo++; ?></td>
                            <td><?php echo h($post['Homesection']['section']); ?></td>
                            <td><?php echo h($post['Homesection']['sections_heading']); ?></td>
                            <td><?php  if($post['Homesection']['content']==1){ echo 'Yes';}else{echo 'No';}?></td>
                            <td><?php  if($post['Homesection']['image']==1){ echo 'Yes';}else{echo 'No';}?></td>
                            <td>
                                <?php if($post['Homesection']['published']=="Published"){ echo $this->Html->link('<span class="fa fa-check"></span>&nbsp;'.__('Published'),array('controller'=>'Homesections','action'=>'published',$id,'Yes'),array('escape'=>false,'class'=>'btn btn-success'));}
                else{echo $this->Html->link('<span class="fa fa-times-circle"></span>&nbsp;'.__('Unpublished'),array('controller'=>'Homesections','action'=>'published',$id,'No'),array('escape'=>false,'class'=>'btn btn-danger'));}?>
                            </td>
                            <td>
                                <div class="btn-group">
