...
<?php }?>
    ...
    <div class="footer-text">&copy; <?php echo$this->Time->format('Y',time());?> <?php echo$siteName;?> <?php echo __('Powered by');?> <?php echo $this->Html->Link( $sitepowerdby,$sitepowerdlink, array('target' => '_blank')); ?></div>
...
else{ echo$this->Html->link($this->Html->image('logo-website.fw.png',array('alt'=>$siteName,'align'=>'center','style'=>'max-width:200px;max-height:77px;')),array('controller'=>'../'),array('escape'=>false));}?></div>
<?php echo $this->fetch('content');?>
</div> <?php }?>
